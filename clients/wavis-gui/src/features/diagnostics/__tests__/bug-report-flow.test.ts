...
describe('Feature: in-app-bug-report, issue link opening', () => {
  it('opens the submitted issue URL via the provided opener', async () => {
    ...
    await expect(openExternalLink('https://github.com/Davalf99/wavis/issues/79', open)).resolves.toBe(true);

    expect(open).toHaveBeenCalledWith('https://github.com/Davalf99/wavis/issues/79');
  });
