...
import {
  ...
  filterSourcesByMode,
  pickInitialMode,
  pickInitialVideoMode,
} from '../SharePicker';
import type { ShareMode, ShareSource, ShareSourceType, ShareSelection } from '../share-types';
...
describe('SharePicker initial mode selection', () => {
  it('prefers audio_only when video tabs are blocked but audio_only is available', () => {
    ...
    expect(pickInitialMode(sources, { videoOccupied: true, audioOccupied: false })).toBe(
      'audio_only',
    );
  });
  ...
  it('video-only source changes never initialize to audio_only', () => {
    ...
    expect(pickInitialVideoMode(sources, { videoOccupied: false, audioOccupied: false })).toBe(
      'window',
    );
  });
  ...
  it('disables companion audio defaults when the audio slot is already occupied', () => {
    ...
    expect(defaultWithAudioForMode('window', { videoOccupied: false, audioOccupied: true })).toBe(
      false,
    );
    expect(
  ...
  it('keeps existing defaults when the audio slot is free', () => {
    ...
    expect(defaultWithAudioForMode('window', { videoOccupied: false, audioOccupied: false })).toBe(
      false,
    );
    expect(
  ...
  it('never returns a blocked mode when an unblocked sourced mode exists', () => {
    fc.assert(
      fc.property(
        fc.array(arbShareSource, { minLength: 1, maxLength: 20 }),
        fc.boolean(),
        fc.boolean(),
        (sources, videoOccupied, audioOccupied) => {
          const mode = pickInitialMode(sources, { videoOccupied, audioOccupied });
          const blocked = mode === 'audio_only' ? audioOccupied : videoOccupied;
          const hasUnblockedSourcedMode = (
            ['screen_audio', 'window', 'audio_only'] as ShareMode[]
          ).some((candidate) => {
