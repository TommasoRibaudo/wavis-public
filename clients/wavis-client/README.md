...
```bash
# With LiveKit audio
cargo run -p wavis-client --features livekit -- --server wss://dt2nm86rf5ksq.cloudfront.net/ws
cargo run -p wavis-client --features livekit -- --server ws://localhost:3000/ws
# Proxy mode only (no LiveKit)
cargo run -p wavis-client -- --server wss://dt2nm86rf5ksq.cloudfront.net/ws
```
...
```bash
./wavis-client --server wss://dt2nm86rf5ksq.cloudfront.net/ws --show-secrets
```
